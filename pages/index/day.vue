<template>
  <view class="container" :class="{ dark: settingsStore.isDarkMode, light: !settingsStore.isDarkMode }">
    ...
    <!-- 模板选择弹窗 -->
    <TemplateSelector v-if="showChooseTpl" :templates="templates" :date="date"
      @close="onCloseTemplateSelector"
      @select-template="onSelectTemplate"
      @save-aerobic="onSaveAerobic"
      @save-rest="onSaveRestDay" />

    <!-- 动作卡片列表 -->
    <scroll-view class="action-list" scroll-y="true" v-if="!isRestDay && !showChooseTpl">
      <ActionCard v-for="(actName, idx) in chosenActions" :key="actName"
        :action-name="actName"
        :entries="actionEntries[idx]"
        :diff="diffs[idx]"
        :latest-record="actionLatestRecordCache[actName] || null"
        :bubble-fill="settingsStore.bubbleFill"
        @confirm-entry="(data) => onConfirmEntry(idx, data)"
        @update-entry="(data) => onUpdateEntry(idx, data)"
        @delete-action="handleDeleteAction(idx)"
        @delete-entry="(eIdx) => handleDeleteEntry(idx, eIdx)"
        @edit-entry="(eIdx) => openEditEntryPopup(idx, eIdx)"
    ...
    <!-- 计时器组件 -->
    <TimerModal :visible="showTimer" :default-duration="timerDuration"
      :quick-settings="quickTimerSettings"
      @close="showTimer = false" @complete="showTimer = false" />

    <!-- 设置组件 -->
    <DaySettings :visible="showSettings" :available-actions="availableActionNames"
      :chosen-actions="chosenActions" :settings="settingsState"
      @close="showSettings = false"
      @add-action="onAddAction"
      @save-sort="onSaveSort"
      @toggle-auto-timer="settingsStore.toggleAutoStartTimer()"
      @toggle-auto-fill="settingsStore.toggleAutoFillData()"
...
import { formatDate, formatDateStr } from '@/utils/theme.js'
import { buildEntry, normalizeEntries, getTotalWeight, ENTRY_TYPE, createPlaceholderEntry, isPlaceholderEntry } from '@/utils/dayHelper.js'
...
export default {
  components: { TimerModal, TemplateSelector, ActionCard, DaySettings },
  data() {
    return {
      ...
      editEntryInfo: { actionIdx: -1, entryIdx: -1 },
      editEntryReps: '',
      editEntryWeight: '',
      // 差异缓存
  ...
  computed: {
    ...
    quickTimerSettings() {
      ...
      return [
        { label: '胸背腿', seconds: h, timeText: fmt(h) },
        { label: '肩手', seconds: l, timeText: fmt(l) },
      ]
    },
  },
  watch: {},
  ...
  methods: {
    ...
    _loadDayData() {
      uni.setNavigationBarTitle({ title: this.date.replace(/-/g, '/') })
    ...
    calcAllDiffs() {
      ...
      this.chosenActions.forEach((actName, idx) => {
        ...
        if (!record) {
          this.$set(this.diffs, idx, { text: '无历史记录', class: 'diff-neutral' })
        } else {
    ...
    updateDiffWithLatestRecord(idx, latestRecord) {
      ...
      if (todayTotal > latestTotal) {
        this.$set(this.diffs, idx, { text: `+${todayTotal - latestTotal}`, class: 'diff-up' })
      } else if (todayTotal < latestTotal) {
        this.$set(this.diffs, idx, { text: `-${latestTotal - todayTotal}`, class: 'diff-down' })
      } else {
        this.$set(this.diffs, idx, { text: '持平', class: 'diff-neutral' })
      }
    },
    debounceCalcDiffs() {
      if (this.calcDiffTimer) clearTimeout(this.calcDiffTimer)
    ...
    persistOrder() {
      const key = this.DAYDATA_PREFIX + this.date
      const raw = uni.getStorageSync(key) || {}
      const dayData = { templates: {}, actions: {}, entries: {}, ...raw }
    ...
    /* ========== 卡片事件处理 ========== */
    onConfirmEntry(idx, { type, stages }) {
      const actName = this.chosenActions[idx]
      ...
      if (this.settingsStore.autoStartTimer) {
        ...
        setTimeout(() => { this.showTimer = true }, 300)
      }
    },
    onUpdateEntry(idx, { entryIdx, entry }) {
    ...
    removeEntry(aIdx, eIdx) {
      ...
      uni.showToast({ title: msg, icon: 'success', duration: 1000 })
    },
    removeActionFromDay(idx) {
      ...
      const tplInfo = dayData.templates[this.chosenTplName] || { actionWeights: {}, totalWeight: 0 }
      ...
      uni.setStorageSync(key, dayData)
      this.dayDataCacheStore.saveDayData(this.date, dayData)
      uni.showToast({ title: `已删除动作：${actNameToRemove}`, icon: 'success' })
      this.calcAllDiffs()
    },
    goHistory(idx) {
      const actName = this.chosenActions[idx]
      uni.navigateTo({ url: `../actionHistory/actionHistory?action=${encodeURIComponent(actName)}` })
    },
    ...
    saveEditedEntry() {
      ...
      const isUnilateral = this.actionStore.getActionByName(actName)?.isUnilateral || false
      const entry = buildEntry(ENTRY_TYPE.NORMAL, [{ reps: repsNum, weight: weightNum }], isUnilateral)
    ...
    onSelectTemplate(name) {
      ...
      dayData.templates[this.chosenTplName] = { ...dayTpl, actionOrder: dayTpl.actionOrder, actionWeights: dayTpl.actionWeights }
    ...
    onSaveAerobic({ name, time }) {
      ...
      const raw = uni.getStorageSync(key) || {}
      const dayData = { templates: {}, actions: {}, entries: {}, ...raw }
      dayData.templates[name] = { totalWeight: time, actionWeights: {}, isAerobic: true }
      uni.setStorageSync(key, dayData)
    ...
    onSaveRestDay(reason) {
      ...
      dayData.templates = { [reason]: { totalWeight: 0, actionWeights: {} } }
      uni.setStorageSync(key, dayData)
      this.dayDataCacheStore.saveDayData(this.date, dayData)
    ...
    onAddAction(actName) {
      if (!this.chosenTplName) {
        uni.showToast({ title: '请先选择训练模板', icon: 'none' })
        return
      }
      ...
      const dayData = { ...raw, templates: raw.templates || {}, actions: raw.actions || {}, entries: raw.entries || {} }
    ...
    onSaveSort(newOrder) {
      ...
      if (orderMap.some(i => i === -1)) {
        uni.showToast({ title: '排序出错，请重试', icon: 'none' })
        return
      }
      ...
      uni.showToast({ title: '排序已保存', icon: 'success', duration: 1000 })
    },
  },
}
</script>
