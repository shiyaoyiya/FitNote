            ...
            <view class="timer-input-group">
              ...
              <view class="timer-input-wrap">
                <input type="number" :value="settings.heavyTimerDuration"
                  @input="$emit('set-heavy-timer', Number($event.detail.value) || 180)"
                  class="timer-input" />
            ...
            <view class="timer-input-group">
              ...
              <view class="timer-input-wrap">
                <input type="number" :value="settings.lightTimerDuration"
                  @input="$emit('set-light-timer', Number($event.detail.value) || 120)"
                  class="timer-input" />
                <text class="timer-unit">秒</text>
              </view>
            </view>
        ...
        <view class="modal-body action-picker-body">
          <view class="search-bar-container">
            <view class="search-bar-inner">
              ...
              <input v-model="searchKeyword" class="search-bar-input" placeholder="搜索动作名称..."
                @input="filterActions" confirm-type="search" />
          ...
          <movable-view v-for="(act, idx) in sortedActions" :key="act" direction="vertical" class="sort-movable-item"
            ...
            <view class="sort-card-wrapper">
              ...
              <view class="sort-card" :style="{ transform: 'translateX(' + (sortSlideX[idx] || 0) + 'px)' }"
                @touchstart="onSortSlideStart($event, idx)"
                @touchmove="onSortSlideMove($event, idx)"
...
export default {
  ...
  props: {
    ...
    settings: { type: Object, default: () => ({ autoStartTimer: false, autoFillData: false }) },
  },
  emits: ['close', 'add-action', 'save-sort', 'toggle-auto-timer', 'toggle-auto-fill', 'toggle-bubble-fill', 'set-heavy-timer', 'set-light-timer'],
  data() {
    return {
...
.no-data-icon { font-size: 40px; margin-bottom: 15px; }
.no-data-text { font-size: 16px; color: #eee; font-weight: bold; margin-bottom: 8px; }
.no-data-sub { font-size: 13px; color: #aaa; }
...
.sort-hint-icon { font-size: 32rpx; }
.sort-hint-text { font-size: 26rpx; color: #aaaaaa; }
.sort-body {
  flex: 1;
  height: 0;
}
