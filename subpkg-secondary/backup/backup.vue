  ...
  <view class="container"
    ...
    <view v-if="activeTab === 'local'" class="tab-content">
      ...
      <view class="bottom-actions">
        ...
        <!-- #ifdef MP-WEIXIN -->
        <button v-if="lastBackupFilePath" class="btn-secondary glass-capsule" @click="handleShareToWechat">
          <text class="btn-icon">💬</text>
          <text>分享到微信聊天</text>
        </button>
        <!-- #endif -->
  ...
  export default {
    ...
    data() {
      return {
        ...
        backupPath: '',
        lastBackupTime: '',
        lastBackupFilePath: '', // 最近一次本地备份的文件路径（用于小程序分享到微信）
        isBackingUp: false,
        isRestoring: false,
        backupProgress: 0,
        backupStatus: {
    ...
    onLoad() {
      ...
      try {
        ...
        const lastFilePath = uni.getStorageSync('last_backup_file_path');
        this.lastBackupFilePath = lastFilePath || '';
    ...
    methods: {
      ...
      async handleCloudBackup() {
        ...
        try {
          ...
          let title = '备份失败'
          if (msg.indexOf('UNAUTHORIZED') >= 0 || msg.indexOf('TOKEN_EXPIRED') >= 0) {
            title = '登录已过期，请重新登录'
          } else if (msg.indexOf('本地服务器不可达') >= 0) {
            title = '服务器未连接'
          } else if (msg) {
            title = '备份失败：' + msg
          }
      ...
      handleStartBackup() {
        ...
        this.$nextTick(async () => {
          try {
            const result = await backupData('full', this.dayDataCacheStore, (progress) => {
            ...
            this.backupPath = getFriendlyBackupPath()
            // 保存最近一次备份文件路径（小程序环境下用于分享到微信）
            if (result && result.path) {
              this.lastBackupFilePath = result.path
              uni.setStorageSync('last_backup_file_path', result.path)
            }
      ...
      handleShareToWechat() {
        // #ifdef MP-WEIXIN
        if (!this.lastBackupFilePath) {
          uni.showToast({ title: '暂无备份文件可分享', icon: 'none' })
          return
        }
        if (!wx.shareFileMessage) {
          uni.showToast({ title: '当前微信版本不支持分享文件', icon: 'none' })
          return
        }
        // 从路径中提取文件名
        const filePath = this.lastBackupFilePath
        const parts = filePath.split('/')
        const fileName = parts[parts.length - 1] || 'fitnote_backup.json'
        wx.shareFileMessage({
          filePath: filePath,
          fileName: fileName,
          success: () => {
