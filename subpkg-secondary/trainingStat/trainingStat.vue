<template>
  <scroll-view class="container"
    ...
    scroll-y @touchstart="onPageTouchStart" @touchmove="onPageTouchMove" @touchend="onPageTouchEnd">
    <view class="content-area">
      <view class="filter-header-row">
        <view class="period-toggle">
          <view class="toggle-btn" :class="{ active: currentPeriod === 'month' }" @click="currentPeriod = 'month'">月
          </view>
          <view class="toggle-btn" :class="{ active: currentPeriod === 'year' }" @click="currentPeriod = 'year'">年
          </view>
        </view>
        <DatePicker :year="currentYear" :month="currentMonth" :period="currentPeriod" :minYear="minYear"
          :minMonth="minMonth" :maxYear="maxYear" :maxMonth="maxMonth" @update:year="onYearChange"
          @update:month="onMonthChange" />
      </view>

      <TrainingOverview :stats="overviewStats" :period="currentPeriod" :dimension="currentDimension"
        :swipeDeltaX="swipeDeltaX" :swipeViewWidth="swipeViewWidth" :noTransition="swipeNoTransition"
        @update:dimension="onDimensionChange" />

      <BodyPartTrend :trendData="trendData" :selectedBodyPart="selectedBodyPart" :period="currentPeriod"
        :dimension="currentDimension" @select:bodyPart="onBodyPartSelect" />

      <BodyPartGrid :bodyPartData="bodyPartGridData" :dimension="currentDimension" :bodyPartOrder="bodyPartOrder"
        :bodyPartVisibility="bodyPartVisibility" :badgeStatus="badgeStatus" @manage="showBodyPartManager = true"
        @select:bodyPart="onBodyPartSelect" />


      <BodyPartManager v-show="showBodyPartManager" @close="onBodyPartManagerClose" @save="onBodyPartManagerSave" />
    </view>
  </scroll-view>
</template>
...
<script setup>
  ...
  import {
    ...
    CATEGORY_NAMES,
    MERGED_CATEGORIES,
    collectAllWeeklyVolume,
    collectCalories
  } from './statUtil.js'
  import {
    rebuildVolumeHistory,
    getStatus
  } from './volumeHistory.js'
  ...
  const caloriesChartData = computed(() => {
    ...
    if (currentPeriod.value === 'year') {
      ...
      return Object.keys(monthBuckets)
        ...
        .map(label => ({
          label,
          kcal: Math.round(monthBuckets[label])
        }))
    }
    return raw.map(item => {
  ...
  function onBodyPartManagerSave({
    order,
    visibility
  }) {
