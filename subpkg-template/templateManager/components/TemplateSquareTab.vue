  ...
  <view class="template-square-tab-container">
    ...
    <scroll-view class="sq-list" scroll-y show-scrollbar="false">
      <view class="sq-list-content" :class="animClass" :key="search + sort + activeTag">
        ...
        <view v-else-if="!loading && isOffline" class="sq-empty">
          <text class="sq-empty-icon">📡</text>
          <text class="sq-empty-text">服务器不可达</text>
          <text class="sq-empty-sub">请连接服务器后重试</text>
          <view class="sq-retry-btn" @click="retry">重试</view>
        </view>
...
<script>
  ...
  } from '@/subpkg-template/utils/templateCache.js'
    ...
    methods: {
      ...
      async loadData() {
        ...
        try {
          ...
          if (list.length > 0) {
            cacheTemplateList({ keyword: this.search, tagId: this.activeTag, sort: this.sort }, { list, total })
          }
          this.isOffline = list.length === 0 && total === 0
          if (this.isOffline) {
            const cached = getCachedTemplateList({ keyword: this.search, tagId: this.activeTag, sort: this.sort })
            if (cached) {
          ...
          this.templates = cached?.list || []
          this.total = cached?.total || 0
          uni.showToast({ title: '网络不可用，显示缓存数据', icon: 'none' })
        } finally {
      ...
      async handleDownload() {
        ...
        try {
          ...
          if (!dataStr) {
            uni.showToast({ title: '服务器不可达，无法下载模板', icon: 'none' })
            return
          }
          ...
          if (this._isMounted) this.isDownloading = false
        }
      },
      async submitShare() {
        ...
        try {
          ...
          if (this._isMounted) this.isSharing = false
        }
      },
    },
  }
