        ...
        <movable-view v-for="(tpl, idx) in filteredTemplates" :key="tpl.id" direction="vertical" class="movable-item"
          ...
          <view class="slide-wrapper">
            ...
            <view class="action-card" :style="{ transform: 'translateX(' + (slideOffset[idx] || 0) + 'px)' }"
              @touchstart.stop="onTouchStart($event, idx)" @touchmove="onTouchMove($event, idx)"
    ...
    <view v-if="showCreatePanel" class="popup-overlay" @click.self="closeCreatePanel">
      ...
      <view class="popup-panel slide-up" @click.stop>
        ...
        <view class="panel-footer">
          <button :class="['btn-confirm', 'glass-btn', { 'glass-btn-enabled': canConfirm && daySettingsStore.liquidGlassEnabled }]" :disabled="!canConfirm" @click="confirmCreate">确认创建</button>
        </view>
      </view>
    </view>
  </view>
...
<script>
  ...
  export default {
    emits: ['open-import-export', 'slideTouchStart', 'slideTouchEnd'],
    data() {
      return {
    ...
    computed: {
      ...
      canConfirm() {
        return this.newTemplateName.trim().length > 0 && this.selectedActions.length > 0
      },
    },
    mounted() {
      this.loadData()
    ...
    methods: {
      ...
      onTouchStart(e, idx) {
        ...
        if (!this.isDragMode) {
          ...
          // 通知父组件：正在滑动卡片，禁止页面切换手势
          this.$emit('slideTouchStart')
        }
      ...
      onTouchEnd(e, idx) {
        ...
        // 通知父组件：卡片滑动结束，恢复页面切换手势
        this.$emit('slideTouchEnd')
      },
      onDragTrigger(idx) {
      ...
      goToTemplateDetail(name) {
        ...
        uni.navigateTo({
          url: `/subpkg-template/templateDetail/templateDetail?template=${encodeURIComponent(name)}`,
          complete: () => {
